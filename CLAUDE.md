...
| `internal/audit` | `audit <disk> <dest-dir> [--tsv f] [--strict]`: report-only content-plausibility of each row's file **tail** (last 64 KiB + size), the torn-write class `verify`/`certify` cannot see (B38, seeded from the Tester's tail scan). Per-type `Classify` → `PLAUSIBLE`/`SUSPECT`/`REVIEW`/`SKIPPED` (JPEG needs an `FF D9` EOI in the tail, else SUSPECT; Sony ARW a whole-MiB size; `.RSV`/`DATABASE.BIN` uniform padding by design; empty `.SRT` twin ok; other types SKIPPED), rows resolved with the same containment/no-symlink walk as `restore`. Opens the manifest read-only; exit 0 with the report, `--strict` exits 1 on any SUSPECT or read error |
