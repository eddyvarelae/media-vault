...
- [ ] **B38** `verify`/`certify` cannot detect a torn write that was hashed after the fact (Tester #24): a hash of a truncated file is still a hash. Add a content-plausibility pass per type (JPEG must end `FF D9`, MP4/MOV atom sizes, ARW/WAV headers; zero-tail / single-byte-tail heuristic) as `vault audit`, report-only; and `copy` must never create a row whose bytes were not compared to a source (the 605 rows of B39 were). Dev, after B40.
- [ ] **B39** 605 `media-sonya6700` rows have an empty `dest_path`, all `_20xx`-named, `copied_at` 2026-04-28 - rows created by scanning the NAS tree itself, not by a copy (Tester #24). Question to Eddy: what ran on the evening of Apr 27/28? Tester's 605-file tail check: 1 zero-tailed (B40), 604 intact. Repair of the rows (fill `dest_path`) after B24's tool, same shape.
- [ ] **B24** 195 `media-sonya6700` rows have `dest_path` missing `CLIP/`/`DCIM/` (Tester #7; bytes verified 195/195). **Tool merged:** `vault repair-dest` on `main` `88d75d7` (Reviewer #7→#10→#15). Next: Tester dry-run on a snapshot (rev 2 item 4) → Reviewer → PM-logged live run on the NAS → verify.
