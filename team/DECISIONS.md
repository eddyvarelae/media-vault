...
## 2026-09-22 17:12 - Eddy: restore the torn `DSC04868_2025.JPG` from `Eddy's Media Vault` "through the Mac Mini"; and "I plan on wipe everything" - make every SSD wipe-ready
- Restore per `runbook-b40-restore.md`: the Mini stages the intact file over SMB into `/volume1/docker/restore-staging/`; `vault restore` runs on the NAS (the manifest is never opened over SMB); verify promotes the row; `media-sonya6700` is re-certified (the Sep-21 cert is archived, not deleted).
- Wipe readiness = a Reviewer-answered rule-8 claim per SSD. kipp (#85) and tars (#91) are answered YES. **B56**: `case` and `Eddy's Media Vault` get the same: Tester re-hashes every file on each SSD against a post-restore manifest snapshot and cross-references each matched row to a certificate entry; Reviewer answers. Until then neither is "ready". Eddy wipes; agents never do.

## 2026-09-22 18:24 - "Every file on `case` / on `Eddy's Media Vault` is on the NAS, verified and certified": YES and YES (Reviewer #93, Codex, after Tester #55)
