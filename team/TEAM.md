...
## Current state (2026-09-23 13:31 - PM-verified, don't re-derive)
...
- `main` tip = see `git log -1`; last code merges = `314416d` (overwrite-guard = `v0.2.1`), `6fbf20a` (kipp-script), `88d75d7` (repair-dest = **`v0.2.2`**, published), `0422b01` (certs-out) = `v0.2.3`, `4a028c8` (tagger), `b1277bd` (restore), `12b5034` (small-fixes), `a4c2ddd` (backup) = **`v0.2.4`**, then `07c32f9` (move exit, `nas-ssd-copy-all.sh <kipp|tars>`), `fc1d828` (audit) = **`v0.2.5`**, `882c165` (toolchain: Go 1.25, alpine 1.25, **linux test CI job** `test.yml` on every push to `main`), `ee6a7fa` (linux-tests), **`4479af3`** (os-root bindings, B43) = **`v0.2.6`** (tagged 16:45, image published 16:54: `v0.2.6`, `sha-4479af3`), then **`2af22d3`** (scripts-v026, #74): every `nas-*.sh` defaults to `v0.2.6` - runbooks and scripts agree, `VAULT_IMAGE` only for a one-off; then **`904d67a`** (docker-cmd, B46): `DOCKER="sudo -n docker"` knob, the `vaultagent` form; then **`d276012`** (kipp-fixes, B47+B48) = **`v0.2.7`** (tagged 2026-09-19 01:44): physical ownership probe, dry-run INCOMPLETE on any unarchived file, kipp under `kipp-<folder>` disk names; then **`7b0ccc8`** (deduped-by-ref, B51) = **`v0.2.8`** (tagged 2026-09-19 14:40): `deduped` rows are by-reference in verify/certify, scripts default `v0.2.8`. `copy` never overwrites a verified destination; `vault repair-dest` exists. Nothing on the NAS has pulled any image since v0.2.0.
- Dev (`~/Projects/media-vault-dev`): rev 7 complete (B43), scripts bump (#74) and B46 DOCKER knob (#77, `904d67a`) merged. B51 (#84, `v0.2.8`) merged. **Idle, nothing assigned** - the remaining Dev-lane items (B39 rows, P2 features) wait on Eddy's decisions or are low priority. Nit for its next note: it stamped #74 `2026-09-19T07:05` - it must read `date`, not guess.
- Tester (`~/Projects/media-vault-tester`, detached): B29 done (Reviewer #8/#14/#20): to archive `kipp` 9,872 / 1.92 TB, `tars` 5,040 / 1.19 TB, EMV 1 file, `case` 0 = 14,913 / 3.11 TB. Found B40 (a certified torn file); archive-wide zero-tail scan running. **B24 live run done 2026-09-18 18:36** (#27-#29, Reviewer #75): `media-sonya6700` is 39,414 verified / 0 copied; backup `manifest.db.bak-b24-20260918-193102` stays on the NAS. kipp identified (`/usb/sdc1`, #31); **kipp dry-run 1 (2026-09-19 01:02, #32) would copy only 4,804 of 9,872 files → B47 (tool bug: cross-root owned check; kept files not INCOMPLETE) + B48 (script: `kipp-*` disk names). **kipp COPIED 2026-09-19 07:36-11:03 (Eddy launched; Tester #41; Reviewer #83 pending): 9,872 files / 1.79 TiB as `kipp-*` rows, 521 deduped, 0 failed, pre-existing 67,735 rows untouched. **B6 for kipp DONE 2026-09-19 13:48-16:29 (Tester #43-#44; Reviewer #85 APPROVE, rule-8 YES): 9,872 verified + 521 by-reference, seven signed certs, cert totals = the disk (10,393 files / 1,953,592,885,376 B). kipp rule-8 = YES (#85). **2026-09-21: B6 media reduced sweep done (8 certs, all 67,729 media-* rows verified; #88); tars on `/usb/sdd1`, B52 merged (`tars-*` names), dry-run 2 APPROVED (#89: 5,039 + 1 out-of-scope `leinfo.sav` = the gap); tars moved to the fast port = `/usb/sdc1` (#50, 894 MB/s); **tars COPIED 2026-09-21 12:27-15:04 (Eddy launched; Tester #52; Reviewer #90/#90b APPROVE): 5,039 files / 1.1 TiB as `tars-*` rows, 2,713 deduped, manifest 85,880 rows, 1 ruled skip. **B6 for `tars-*` DONE 2026-09-21 18:16-20:05 (Tester #53; Reviewer #91 APPROVE, tars rule-8 = YES). Now: all 19 disks with rows are verified + certified; open: B53 weekly job, B54 in-tree certs (Eddy's rm), B55 second suffix, B39 605 rows, B40 restore, B18/B22 LaunchAgents; wipe/keep of kipp and tars = Eddy.** Root weekly cron disabled by Eddy 00:37 (B53/B54 open).**
- NAS: DXP2800 `192.168.1.167`, SMB + SSH as `figmaboi` (key); **agent account `vaultagent` (key-only, `sudo docker` NOPASSWD only) since 2026-09-18 18:30 - use it for every NAS command after B24;** `kipp` is on the NAS (USB-C, likely `/mnt/@usb/sdc1`, confirm by content); Eddy's kipp precaution copy on Scratch1 is untouchable; `~/mounts/media` and `~/mounts/docker` mount via `com.varela.mount-nas`. NAS clock is UTC-6 (Mini is UTC-7). `sudo docker` is password-less; so is `sudo bash` (B28).
- Scope since 2026-09-17: every scheduled batch against the archive is ours (nightly tagger = B17, after rev 3). **Found 2026-09-21: a ROOT cron (`/etc/cron.d/vault-weekly-verify`, Sun 02:00) still runs the April verify+certify with `:latest` and in-tree certs - Eddy to disable; B53/B54. It explains the Sep 13 pass and re-hashed six disks clean on Sep 20.**
- **P0s: 2,668 SonyA6700 photos overwritten Sep 1 are LOST (B23a closed; B23b fixed in v0.2.1). One certified NAS file is a torn write (B40, recovery path being built). B24 repair done Sep 18: the SonyA6700 folder verifies clean.** Archive is 8.20 TiB (not 3.3). Agents never wipe or write an SSD.
- **Disk swap 2026-09-23:** Eddy erases `Scratch1` back to Time Machine and formats `case` (rule-8 YES #93) as the new `Scratch1`. The Tester's evidence folder `/Volumes/Scratch1/tester/` is mirrored to `/volume1/docker/tester-artifacts/` first (rev 17) and restored to the new disk after. All four SSDs are wipe-ready on the record (kipp #85, tars #91, case + EMV #93).
...
- Reviewer = `codex exec … </dev/null`, run by the PM. Codex has a usage limit: hit 2026-09-17 20:21 (→21:34), 22:30 (→02:35), 2026-09-18 03:53 (→07:38), 10:49 (→12:41), 16:50 (→17:44), then 2026-09-19 16:39 the weekly limit; **Eddy upgraded Codex to the 5x plan 2026-09-21 00:2x** (#85 ran at once). Second reviewer for numbers only: Gemini CLI `gemini-3.8-flash` (`team/context/reviewer-gemini.md`), free tier = 20 req/day, useless without billing; queue them, never two at once. Seats boot in visible Terminal windows with `--remote-control`; nudges need a trailing empty `do script`.
