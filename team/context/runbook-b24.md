...
- Tool: `vault repair-dest <disk> <dest-root> [--dry-run]` on `main` `88d75d7` (Reviewer #15); published as **`v0.2.2`** (tag on `7d7286c`). It rewrites `dest_path` only, one row at a time, after a size + sha256 match; exits 1 if any row stays unresolved.
...
3. **Dry-run on the live manifest** (confirms the image and the mounts; writes no row):
   `sudo docker run --rm -v /volume1:/volume1 -e VAULT_CONFIG=/volume1/docker/vault-nas-config ghcr.io/eddyvarelae/media-vault:v0.2.2 repair-dest media-sonya6700 /volume1/media/SonyA6700 --dry-run`
...
5. **Verify**: `… ghcr.io/eddyvarelae/media-vault:v0.2.2 verify media-sonya6700 /volume1/media/SonyA6700 --only-unverified` → `Verified: 195 … Missing: 0`, exit 0 (this is the first B6 step; ~1.15 GB hashed, minutes).
