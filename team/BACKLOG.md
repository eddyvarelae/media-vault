...
- [ ] **B29** **Run media-vault from the Mac mini against Mini-attached SSDs** (Eddy's goal, 2026-09-17). Today `vault` runs on the NAS in docker with SSDs on the NAS USB ports and the manifest local to it. From the Mini the archive is SMB and the manifest is a WAL sqlite that must never be opened over SMB - the Sep 1-3 Mini-side runs did exactly that (Tester #18: `.bak` files `figmaboi`-owned). PM designs the safe shape before any Mini-side write: candidates are (a) SSDs moved to the NAS USB and runs launched over SSH (`sudo docker run`, no design change), (b) `vault` on the Mini with the manifest pulled to Scratch1, written locally, pushed back under a lock (single-writer preserved by protocol). Decision item once B23(b)/B24 land; `kipp` (B26) is the first real run.
