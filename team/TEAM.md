...
## Current state (2026-09-18 15:56 - PM-verified, don't re-derive)
...
- `main` tip = see `git log -1`; last code merges = `314416d` (overwrite-guard = `v0.2.1`), `6fbf20a` (kipp-script), `88d75d7` (repair-dest = **`v0.2.2`**, published), `0422b01` (certs-out) = `v0.2.3`, `4a028c8` (tagger), `b1277bd` (restore), `12b5034` (small-fixes), `a4c2ddd` (backup) = **`v0.2.4`**, then `07c32f9` (move exit, `nas-ssd-copy-all.sh <kipp|tars>`), `fc1d828` (audit) = **`v0.2.5`**, `882c165` (toolchain: Go 1.25, alpine 1.25, **linux test CI job** `test.yml` on every push to `main`), **`ee6a7fa`** (linux-tests: fixtures FS-aware; `main` green on Linux). Scripts default `v0.2.5`. `copy` never overwrites a verified destination; `vault repair-dest` exists. Nothing on the NAS has pulled any image since v0.2.0; scripts default to `v0.2.0` on `main` until `certs-out` (B37) lands - runbooks pass `VAULT_IMAGE`.
- Dev (`~/Projects/media-vault-dev`): rev 7 PR-A merged; building PR-B `os-root` (keep the Root-anchored walk + os.Root escape backstop, PM ruling 15:44).
