...
// `X.mov` and `x.mov` are one file on a case-insensitive root, or that a
// `../archive/x` rule spelling is `archive/x`. dest_path is relative to a
// root the manifest does not record; the caller joins it to its own.
func (m *Manifest) VerifiedRows() ([]Entry, error) {
	rows, err := m.db.Query(`
		SELECT source_disk, source_path, dest_path, size, mtime_ns, sha256,
		       copied_at, COALESCE(verified_at, 0), status
		FROM files
