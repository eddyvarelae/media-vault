...
| `internal/audit` | `audit <disk> <dest-dir> [--tsv f] [--strict]`: report-only content-plausibility of each row's file **tail** (last 64 KiB + the size read from the open fd), the torn-write class `verify`/`certify` cannot see (B38, seeded from the Tester's tail scan). Per-type `Classify` → `PLAUSIBLE`/`SUSPECT`/`REVIEW`/`SKIPPED` (JPEG: an `FF D9` EOI then only zeros to EOF, else SUSPECT; Sony ARW a whole-MiB size else REVIEW; `.RSV`/`DATABASE.BIN` uniform padding by design; empty `.SRT` PLAUSIBLE only with a video twin on the disk else REVIEW; other types SKIPPED, named `ext:count`). Every read is anchored to an `os.Root` on the dest dir (B43): the no-symlink walk (`scan.SymlinkComponentRoot`) and the leaf `Lstat`/open resolve through the pinned fd, `os.Root` refuses any escaping component, and the leaf is confirmed the `Lstat`'d file by `SameFile` (not `O_NOFOLLOW`, since `os.Root` follows an in-root leaf symlink); a short read is an ERROR. Residual (os.Root doc): a parent swapped to an *in-root* symlink after the walk is followed — `os.Root` blocks only escapes. Opens the manifest read-only; exit 0 with the report, `--strict` exits 1 on any SUSPECT or read error |
...
| `scripts/*.sh` | How work runs on the NAS: `docker run --rm … ghcr.io/eddyvarelae/media-vault:<tag> <command>`, sequential, as root via `sudo nohup`; every script defaults `IMG` to the pinned release tag. Logging goes through a `log()` helper (once per line under any launch form — B27/B35). `nas-ssd-copy-all.sh <label>` (B26/B36) copies one source SSD folder-by-folder: `label` is `tars` (source `/usb/sdc1`, 4 cameras, no dedupe) or `kipp` (`SSD_SRC` required — the disk's container path — 7 folders, `--dedupe-content`); `DRY_RUN=1` plans only; per-folder routing flags and the per-label tables per `team/context/runbook-kipp.md` step 2 |

## Manifest status vocabulary

`copied` → `verified` (or `mismatch`), plus `deduped` (content already
archived under another row; `dest_path` points at the **owner's**) and
`inventoried` (NAS-side row, no `dest_path`). `copy` and recopy write `copied`;
only `verify` promotes.

`deduped` rows are **by reference** (B51): the bytes live on a verified row of
some disk, and the deduped row's `dest_path` is the owner's, resolved under the
owner's root — not necessarily this disk's. So `verify` **skips** them (no
hashing under this root, no status change — hashing here would read the wrong
root and count a spurious `Missing`, and promoting would destroy the
provenance) and reports them as `Deduped (by reference): N`. `certify` accepts a
`deduped` row **iff** its sha is on a `verified` row of any disk (else
`ErrNotCertifiable`, naming the row); the certificate entry records that owner
as `by_reference: {disk, dest_path}` and carries the owner's `verified_at`.
`certify` requires **every** non-`deduped` row `verified` — the fast route to
that is verifying, never editing status.
...
from the root with `Lstat` — in `scan.Build` before admitting
(`scan.SymlinkComponent`), and again in `copy.File` before writing, there
re-anchored to an `os.Root` on the dest (`scan.SymlinkComponentRoot`) so the
walk and every write op share one pinned fd and `os.Root` refuses any
escaping component outright (B43) — and a symlink component refuses the file
(`DstThroughLink`, never written). A component
...
for, and never claims a row it cannot back with a hash. `copy.File` is the
only path that mints a row, and its sha is the hash of the bytes it streamed
from the source (the tee), never a stat/scan of the destination after the
fact — an assertion refuses to return a row otherwise (B38). A row whose
bytes were never compared to a source (the B39 empty-`dest_path` import rows,
the B40 torn write) is exactly what `vault audit` exists to surface.
...
| `audit` | unknown flag or `--tsv` without a value (`die`); wrong arity; TSV write error (`die`); with `--strict`, any `SUSPECT` or per-row read error (exit 1) | without `--strict` it is report-only: SUSPECT/REVIEW/ERROR rows are listed and counted on the `AUDIT …` line but exit is 0; a `--tsv` of the findings |
...
- Atomic destination writes only (`.vault-partial` created `O_EXCL` → fsync
  → rename). The staging path must be empty; the writer never truncates. Every
  write/read that resolves a caller-supplied path under a root — `copy.File`,
  `certify.WriteOutput`, `audit`, `restore` — is anchored to an `os.Root` on
