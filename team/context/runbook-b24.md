...
Status: **EXECUTED 2026-09-18 18:36** by the Tester as `figmaboi` (Tester #27-#29, Reviewer #75 APPROVE). Kept for the record and as the template for future manifest repairs.
...
2. **Backup the manifest** (root-owned file): `sudo -n bash -c 'cp -p /volume1/docker/vault-nas-config/manifest.db /volume1/docker/vault-nas-config/manifest.db.bak-b24-$(date +%Y%m%d-%H%M%S)'` and `sha256sum` both. (`figmaboi`'s NOPASSWD covers only `docker`, `nohup`, `bash` - plain `sudo cp` prompts; Tester #27.)
...
## Rollback
`sudo -n bash -c 'cp -p <the .bak from step 2> /volume1/docker/vault-nas-config/manifest.db'` (only while no vault process runs).
