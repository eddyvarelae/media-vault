...
Status: **drafted; waits on Eddy's go** (asked 2026-09-19 11:46). Executor: Tester as `vaultagent` under a PM deploy lock. Writes: manifest status rows (`copied`→`verified`, `verified_at`), seven cert files under `/volume1/docker/vault-certs/`, appends to `/volume1/docker/verify-certify.log`, one manifest backup. Nothing under `/volume1/media` is written.
...
2. **Run**, detached, as one loop (six copied disks first, `kipp-leantank` last):
   ```
   ssh vaultagent@192.168.1.167 'cd /volume1/docker && nohup bash -c '"'"'
   IMG=ghcr.io/eddyvarelae/media-vault:v0.2.7; LOG=/volume1/docker/verify-certify.log; CERTS=/volume1/docker/vault-certs
   for e in kipp-sonya6700:SonyA6700 kipp-backup:Backup kipp-multicam:Multicam kipp-auditorium:Auditorium kipp-gopro:GoPro kipp-sonyzve10:SonyZVE10 kipp-leantank:LeanTank; do
     d=${e%%:*}; r=/volume1/media/${e#*:}
     echo "[$(date)] === VERIFY $d at $r ===" >> $LOG
     sudo -n docker run --rm -v /volume1:/volume1 -e VAULT_CONFIG=/volume1/docker/vault-nas-config $IMG verify $d $r --only-unverified >> $LOG 2>&1 || { echo "[$(date)] VERIFY FAILED for $d - skipping certify" >> $LOG; continue; }
     echo "[$(date)] === CERTIFY $d ===" >> $LOG
     sudo -n docker run --rm -v /volume1:/volume1 -e VAULT_CONFIG=/volume1/docker/vault-nas-config $IMG certify $d $CERTS/$d.cert.json --root $r >> $LOG 2>&1 || echo "[$(date)] CERTIFY FAILED for $d" >> $LOG
     echo "[$(date)] $d done" >> $LOG
   done; echo "[$(date)] kipp verify+certify pass done" >> $LOG'"'"' >/dev/null 2>&1 &'
   ```
   Note the log byte offset before starting. This is the `nas-verify-certify-all.sh` loop restricted to the kipp disks, inlined because the script's kipp entries are commented out and it has no disk filter (B50, Dev, later).
