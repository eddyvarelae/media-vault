...
3. **Dry-run on the live manifest** (confirms the image and the mounts; writes no row):
   `sudo docker run --rm -v /volume1:/volume1 -e VAULT_CONFIG=/volume1/docker/vault-nas-config ghcr.io/eddyvarelae/media-vault:v0.2.3 repair-dest media-sonya6700 /volume1/media/SonyA6700 --dry-run`
...
5. **Verify**: `… ghcr.io/eddyvarelae/media-vault:v0.2.3 verify media-sonya6700 /volume1/media/SonyA6700 --only-unverified` → `Verified: 195 … Missing: 0`, exit 0 (this is the first B6 step; ~1.15 GB hashed, minutes).
