...
1. Merge to `main` only through `team/channels/review-requests.md` (APPROVE first). A merge publishes no **image**: `docker.yml` runs on `v*` tags and manual dispatch only, and never publishes `:latest` — `docker/metadata-action` is pinned to `flavor: latest=false`, because its default (`latest=auto`) adds `:latest` to every stable semver tag even with no `latest` line in `tags:`. (`test.yml` — the Go suite, no image — does run on every push/PR to `main` and on `v*` tags.)
2. The PM tags the release on the `main` tip: `git tag v0.2.1 && git push origin v0.2.1`. CI (`.github/workflows/docker.yml`) publishes `ghcr.io/eddyvarelae/media-vault:v0.2.1` (plus `:sha-<commit>`) for linux/amd64 + arm64.
