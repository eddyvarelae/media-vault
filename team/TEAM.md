...
- `main` tip = see `git log -1`; last code merges = `314416d` (overwrite-guard = **`v0.2.1`**, tagged 21:39), `6fbf20a` (kipp-script, #19), **`88d75d7`** (repair-dest, #15). `copy` never overwrites a verified destination; `vault repair-dest` exists. Nothing on the NAS has pulled any image since v0.2.0; scripts default to `v0.2.0` on `main` until `certs-out` (B37) lands - runbooks pass `VAULT_IMAGE`.
